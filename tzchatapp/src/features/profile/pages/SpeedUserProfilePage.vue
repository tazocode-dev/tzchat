  ...
  <div class="page-wrapper">
    <div class="container">
      ...
      <div class="card pf-scope">
        <div class="action-grid" role="group" aria-label="사용자 액션">
          ...
          <ion-button
            ...
            aria-label="이 사용자와 대화하기"
            :disabled="!user.isFriend || user.isBlocked || isSubmitting"
      ...
      <UserReportModal
        ...
        :user-id="String(user._id || route.params.id || '')"
...
import { IonButton as ionButton, IonIcon, toastController } from '@ionic/vue'
...
import SpeedMatchRequestModal from '@/features/friends/components/SpeedMatchRequestModal.vue'
...
async function showChatError(message: string) {
...
async function startChat(targetId: string) {
  ...
  if (!user.value.isFriend || user.value.isBlocked) {
    await showChatError('친구이며 차단하지 않은 사용자와만 대화할 수 있습니다.')
    return
  }
  if (!/^[a-f\d]{24}$/i.test(normalizedTargetId)) {
    await showChatError('대화 상대 정보를 확인할 수 없습니다.')
    return
  }
  ...
  try {
    ...
    const roomId = String(response?.data?._id || response?.data?.id || '')
    if (!/^[a-f\d]{24}$/i.test(roomId)) throw new Error('INVALID_CHAT_ROOM_ID')
    await router.push(`/home/chat/${roomId}`)
  } catch (error: any) {
    const serverMessage = error?.response?.data?.message || error?.response?.data?.error
    await showChatError(serverMessage || '채팅방을 열지 못했습니다. 잠시 후 다시 시도해 주세요.')
  } finally {
