<template>
  <div class="page-wrapper">
    ...
    <!-- 모달들 (lazy-load) -->
    <RegionEditModal v-if="showModal1" :message="popupMessage" @close="showModal1 = false" @updated="handleRegionUpdate" />
    <PreferenceEditModal
      v-if="showModal2"
    ...
    />
    <SelfIntroductionEditModal v-if="showModal3" :message="popupMessage" @close="showModal3 = false" @updated="handleIntroUpdate" />
    <NicknameEditModal v-if="showModal4" :message="popupMessage" @close="showModal4 = false" @updated="handleNicknameUpdate" />
    <MarriageEditModal v-if="showMarriageModal" :message="user?.marriage || ''" @close="showMarriageModal = false" @updated="handleMarriageUpdated" />

    <SearchAgeRangeEditModal
      v-if="showSearchYear"
    ...
    />
    <SearchRegionsEditModal v-if="showSearchRegion" :regions="regionsForModal" @close="showSearchRegion = false" @updated="onSearchRegionUpdated" />
    <SearchPreferenceEditModal v-if="showSearchPreference" :message="user?.search_preference ?? ''" @close="showSearchPreference = false" @updated="onSearchPreferenceUpdated" />
    <SearchMarriageEditModal v-if="showSearchMarriage" :message="user?.search_marriage ?? '전체'" @close="showSearchMarriage = false" @updated="handleSearchMarriageUpdated" />
...
const RegionEditModal = defineAsyncComponent(() => import('@/features/profile/components/RegionEditModal.vue'))
const PreferenceEditModal = defineAsyncComponent(() => import('@/features/profile/components/PreferenceEditModal.vue'))
const SelfIntroductionEditModal = defineAsyncComponent(() => import('@/features/profile/components/SelfIntroductionEditModal.vue'))
const NicknameEditModal = defineAsyncComponent(() => import('@/features/profile/components/NicknameEditModal.vue'))

const SearchAgeRangeEditModal = defineAsyncComponent(() => import('@/features/profile/components/SearchAgeRangeEditModal.vue'))
const SearchRegionsEditModal = defineAsyncComponent(() => import('@/features/profile/components/SearchRegionsEditModal.vue'))
const SearchPreferenceEditModal = defineAsyncComponent(() => import('@/features/profile/components/SearchPreferenceEditModal.vue'))

const PasswordChangeModal = defineAsyncComponent(() => import('@/features/profile/components/PasswordChangeModal.vue'))
const ProfilePhotoManager = defineAsyncComponent(() => import('@/features/profile/components/ProfilePhotoManager.vue'))

const MarriageEditModal = defineAsyncComponent(() => import('@/features/profile/components/MarriageEditModal.vue'))
const SearchMarriageEditModal = defineAsyncComponent(() => import('@/features/profile/components/SearchMarriageEditModal.vue'))
...
async function ensureSensitivePreferenceConsentForUse(){
  try {
    ...
    console.error('민감정보 선택 동의 확인 실패')
...
const logout = async () => { try { await axios.post('/api/logout', {}, { withCredentials: true }); router.push('/login') } catch { console.error('로그아웃 실패') } }
</script>
