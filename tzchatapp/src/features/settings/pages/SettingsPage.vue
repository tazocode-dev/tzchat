...
async function getAppVersion() {
  if (!Capacitor.isNativePlatform()) return __APP_VERSION__
  try {
    const info = await App.getInfo()
    return info.version || String((info as any).build) || __APP_VERSION__
  } catch {
...
async function openSupportMail() {
  if (!me.value) {
    try { await loadMe() }
    catch { console.error('❌ 문의 사용자 정보 조회 실패') }
  }
