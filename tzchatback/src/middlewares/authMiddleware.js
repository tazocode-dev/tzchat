...
// - ✅ 공개(무인증) 경로 화이트리스트 추가: 로그인/회원가입/헬스체크 등
...
module.exports = async function authMiddleware(req, res, next) {
  try {
    ...
    // 상위 라우터에서 이미 인증과 사용자 조회가 끝난 경우 중복 DB 조회를 피한다.
    if (req.user?._id && req.auth?.userId) return next();
    ...
    const me = await (User.findById(userId).lean
      ? User.findById(userId).select('_id username nickname role roles permissions isAdmin isMaster email phone birthDate birthyear gender profileOnboardingCompletedAt').lean()
      : User.findById(userId).select('_id username nickname role roles permissions isAdmin isMaster email phone birthDate birthyear gender profileOnboardingCompletedAt'));
    if (!me) {
    ...
    if (forcedRole && String(me.role || '').toLowerCase() !== forcedRole) {
      await User.updateOne({ _id: me._id }, { $set: { role: forcedRole } });
      me.role = forcedRole;
    }
